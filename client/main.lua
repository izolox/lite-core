...
Core.RunParachuteTimer = function()
    CreateThread(function()
        local timer = 0

        Utils.DebugPrint('INFO', 'Parachute timer: '..timer)
        while IsPedFalling(cache.ped) do
            if timer >= 1 then
                TriggerServerEvent('lite-core:giveChute')
                break
            end
    ...
    CreateThread(function()
        while true do
            if IsPedFalling(cache.ped) then
                if not Core.InFreeFall then
                    Core.InFreeFall = true
                    Core.RunParachuteTimer()
                end
            end
            Wait(1000)
        end
